...
smallestGroupSize <- 3
idx <- rowSums(counts(dds_all) >= filt) >= smallestGroupSize
...
for (i in seq_along(dds)) {
  saveRDS(dds[[i]],
    file = paste0(
      dir_output,
      format(Sys.Date(), "%y%m%d"), "_", "dds", "_", names(dds[i]),
      ".rds"
    )
  )
}
...
for (i in names(counts)) {
  write.csv(
    counts[[i]],
    file = paste0(dir_output, format(Sys.Date(), "%y%m%d"), i, ".csv"),
    row.names = TRUE
  )
}
...
for (i in names(vsd)) {
  saveRDS(
    ...
    file = paste0(
      dir_output,
      format(Sys.Date(), "%y%m%d"), "_", "vsd", "_", i,
      ".rds"
    )
  ) # nolint
}